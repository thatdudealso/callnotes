...
- iOS targets are not built in CI yet, although the Phase 6 app and Share Extension now carry real code. An iOS Simulator build job (`-scheme CallNotesIOS -destination 'generic/platform=iOS Simulator'`, code signing disabled) still needs to be added; until then the phone targets are only built locally, and the Phase 6 sync logic that CI does cover is the part that lives in `CallNotesCore` (pairing, the upload inbox, mirror reconciliation).
