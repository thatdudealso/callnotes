...
- Only CallNotes' own capture CAF is L = near / R = far (`FileAudioLoader.ChannelLayout.captureNearFar`); any other two-channel import is `unknownStereo`, downmixed to `mixed`, and diarized, so an imported podcast is not mislabeled as owner/counterparty.
